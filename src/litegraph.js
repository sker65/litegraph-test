(function (global) {
    ...
    LGraph.prototype.getListOfGraphCanvas = function () {
    ...
    LGraph.prototype.getNodes = function (id) {
        return this._nodes;
    };
...
*/
    ...
    LGraphNode.prototype.resize = function () {
        ...
        if (e.which == 1) {
            ...
            if (this.dragging_rectangle) {
                ...
                if (
                    ...
                    LGraphCanvas.onShowPropertyEditor({ property: "title", type: "string" }, null, null, null, node);
                }
	...
	*/
        ...
        for (var i = 0; i < widgets.length; ++i) {
            ...
            switch (w.type) {
                ...
                case "slider":
                    ...
                    if (show_text) {
                        var text = w.name + "  " + Number(w.value).toFixed(w.options.prec === undefined ? 2 : w.options.prec);
                        if (w.options.formatter) {
                            text = w.options.formatter(w);
                        }
                        ctx.textAlign = "center";
                        ctx.fillStyle = text_color;
                        ctx.fillText(
                ...
                case "text":
                    ...
                    if (show_text) {
                        ...
                        var txt = w.value;
                        if( w.options.json) {
                            txt = w.error ? "bad JSON":"JSON";
                        }
                        while( ctx.measureText(txt).width > maxWidth ) {
                            txt = txt.substr(1);
                        }
                        ctx.fillText(txt, maxWidth, y + H * 0.7);
                    }
                    break;
                default:
                    if (w.draw) {
    ...
    LGraphCanvas.prototype.processNodeWidgets = function (
        ...
        function inner_value_change(widget, value) {
            ...
            if (widget.callback) {
                widget.callback(widget.value, that, node, pos, event, widget);
            }
        }
    ...
    LGraphCanvas.prototype.prompt = function (title, value, callback, event, options) {
        ...
        var html = "<span class='name'></span> <input id='promptin' autofocus type='text' class='value'/><button class='rounded'>OK</button>";
        if( options.big ) {
            html = "<span class='name'></span> <textarea  id='promptin' rows='6' cols='30' autofocus class='value'/>"+value+"</textarea><button class='rounded'>OK</button>";
        }
        ...
        var input = dialog.querySelector("#promptin");
        input.addEventListener("keydown", function (e) {
            modified = true;
            if (e.keyCode == 27) {
                //ESC
    ...
    LGraphCanvas.prototype.showEditPropertyValue = function (
        ...
        if (type == "string" || type == "number" || type == "array") {
            if( options.big ) {
                input_html = "<textarea rows='6' cols='40# autofocus class='value'/>"+node.properties[property]+"</textarea>";
            } else {
